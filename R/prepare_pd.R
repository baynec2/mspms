...
prepare_pd <- function(filepath) {
  ...
  if (!all(c(
    ...
    stop("This does not look like a proteome discoverer file.
         The file should contain columns named: Annotated Sequence,
         Master Protein Accessions, Qvality PEP")
  }
  ...
  peptide_fixed <- name_fixed %>%
    ...
    dplyr::mutate(Peptide = gsub("\\]", "", .data$Peptide)) %>%
    dplyr::mutate(Peptide = gsub("\\[", "", .data$Peptide)) %>%
    # Only keep max quality peptide, when there is more than one match.
