...
plot_qc_check <- function(processed_qf,
  ...
  qc_check_data = prepare_qc_check_data(processed_qf,
                                        peptide_library)
  
  # Plotting per
  p1 <- qc_check_data %>%
    ggplot2::ggplot(ggplot2::aes(color = .data$peptide_type,
    ...
    ggplot2::geom_vline(xintercept = full_length_threshold, linetype = "dashed",
                       color = "#00BFC4") +
    ggplot2::geom_vline(xintercept = cleavage_product_threshold, linetype = "dashed",
                        color = "#F8766D")+
