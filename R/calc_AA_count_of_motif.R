...
#'
#' @param cleavage_motif this is a vector of cleavage motifs that you would like to count the amino acids for.
...
calc_AA_count_of_motif = function(cleavage_motif){
  ...
  AAs = tidyr::separate(
    sequences,
    col = 1,
    into = c("exclude",paste0("position", 1:(nchar))),
    sep = "",
    remove = TRUE
  ) %>%
    dplyr::select(-.data$exclude)

  # Define the desired order. This ensures that the amino acids will always be in the same order.
  desired_order = c('A', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'n',
                    'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'X', 'Y')
