...
#' file.
#' @param pd_data PeptideGroups.txt file generated by DIA-NN and read
#' into R.
#'
#' @return a stop command with a informative message if file looks unexpected.
#' otherwise, nothing.
check_file_is_valid_diann <- function(diann_data){
  expected_names <- c("Protein.Group","Protein.Ids","Protein.Names","Genes",
  ...
  names_in_data <- names(diann_data)
  # What names are missing
  missing_names <- expected_names[expected_names %!in% names_in_data]
    if (length(missing_names) > 1) {
    stop(
