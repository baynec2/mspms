...
#'
#' calculate all possible cleavages for a defined peptide library containing peptides of the same length.
#'
#' @param peptide_library_seqs = these are the sequences from the peptide library. Should be the same length
#' @param n_AA_after_cleavage = this is the number of AA after cleavage sites you want to produce peptides for.
#'
#' @return a vector of all the possible cleavages for the peptide library sequences
#' @export
#'
#' @examples
#' calculate_all_cleavages(mspms::peptide_library$library_real_sequence,
#'                         n_AA_after_cleavage=4)
calculate_all_cleavages = function(peptide_library_seqs,n_AA_after_cleavage=4){
  ...
  for(i in seq_len(length-1)){
    ...
    second_fragment = substr(peptide_library_seqs,i+1,i +n_AA_after_cleavage)
