add_cleavages = function(joined_with_library){
  ...
  cleavages = bind_cols(nterm,cterm[,2:3])


  # Building final data frame.
  output = dplyr::bind_cols(joined_with_library,cleavages) %>%
    dplyr::select(library_reference_id,library_real_sequence,Peptide,nterm,nterm_cleavage_pos,cterm,cterm_cleavage_pos,dplyr::everything())
